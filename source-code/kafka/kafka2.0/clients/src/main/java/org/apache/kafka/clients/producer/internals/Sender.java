...
public class Sender implements Runnable {
    ...
    private final org.apache.kafka.clients.producer.internals.RecordAccumulator accumulator;
    ...
    public Sender(LogContext logContext,
                  KafkaClient client,
                  Metadata metadata,
                  org.apache.kafka.clients.producer.internals.RecordAccumulator accumulator,
                  boolean guaranteeMessageOrder,
                  int maxRequestSize,
                  short acks,
                  int retries,
                  org.apache.kafka.clients.producer.internals.SenderMetricsRegistry metricsRegistry,
                  Time time,
    ...
    void run(long now) {
        if (transactionManager != null) {
            try {
                ...
                // 如果producer只开启了幂等(没开启事务)
                if (!transactionManager.isTransactional()) {
                    // this is an idempotent producer, so make sure we have a producer id
                    // 并且还没有producerID，那么就会去broker上获取一个producerID和epoch
        ...
        // 将可以发送的消息批次封装成一个请求，并且将这个请求入队(并不是真正意义上的入队，只是等同于入队)
        long pollTimeout = sendProducerData(now);
        // 发送队列中(不是真正意义上的队列)的请求，并且获取broker响应的消息
    ...
    private long sendProducerData(long now) {
        ...
        // 获取可以发送的批次 这个方法会去遍历各个TopicPartition下的批次队列中的第一个消息批次
        // 符合可以发送的条件如下 只要符合一个就可以发送
        // 1. 如果批次队列的长度大于1或者说只有一个批次但是批次已满
        // 2. 等待长达linger.ms时间的批次
        // 3. 对应的批次队列中存在因为内存不足而阻塞在加入消息批次过程中的消息 (比如说发送了2条消息，消息A和消息B，先为消息A分配好了批次,并且加入队列
        // ，然后在为消息B分配批次的时候，由于客户端缓冲区不够分配了，所以阻塞住)
        // 这边最终获取到的是要发送的话题分区所在的broker地址
        org.apache.kafka.clients.producer.internals.RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now);

        // 如果存在一些已经准备好发送的消息不知道发往哪个broker，那么就去更新一下topic的元数据，也就是获取所需要发送的分区的信息
        ...
        // 检测这些可以发送的消息对应的broker的会话状态
        // 也就是通过selector检测channel是否就绪，如果尚未就绪就先不发送这些消息
        ...
        // 此处传入的readyNodes是一个个可以发送的broker节点
        // drain方法返回的batches是以broker.id为key，消息批次集合为value
        // 此处的消息批次列表不一定是同一个批次，比如可能有对应topicA分区0的消息批次A，topicB分区1的消息批次B
        // 他们并不是同一个消息批次队列里面的，但是刚好他们要发送的分区都在同一个broker上，
        // 所以说生产者是会将要发送往同一个broker节点上的准备好的消息批次都放进一个列表在进行发送
        // (放进去会判断队列当前大小是否大于max.request.size，如果大于就不会再放，等待下一次在发送)

        // 如果开启了幂等，那么在brain方法中会为每个还没序列号的批次的生成一个序列号,每个分区的序列号从0开始
        // 而如果消息批次已经有了序列号，说明这个消息批次是一个正在重试的消息批次(因为默认每个批次在创建时都是没有序列号的，需要在brain方法中创建
        // ,在brain方法之前就已经拥有序列号，说明这个消息已经经历过上一轮的brain方法了)
        // 比如消息批次A的序列号为100，然后在成功写入到brokerA的topicA的分区0中，但是由于延迟，broker没有及时响应成功
        // 那么此时消息批次A会重新发送，序列号依旧还是100，但是brokerA上发现topicA的分区0上已经存在了序列号为100的消息批次，就会拒绝这次写入
        // 这也就是幂等性实现的原理，但是这种幂等性只能针对于分区级别
        Map<Integer, List<org.apache.kafka.clients.producer.internals.ProducerBatch>> batches = this.accumulator.drain(cluster, result.readyNodes,
                this.maxRequestSize, now);
        // 此处逻辑是保证了各个分区的消息顺序性(max.in.flight.requests.per.connection=1的情况下guaranteeMessageOrder为true)
        // 举个例子，比如有消息批次A属于topicA的分区0，那么经过下面的逻辑之后会被记录到一个map中，这个map类似于屏蔽的功能
        // 比如map中将消息批次A对应的topicA的分区0屏蔽，那么下次如果有一个消息批次B准备好了也要发送，那么在this.accumulator.drain方法中会去判断，这个消息批次B
        // 对应的话题分区是否有被屏蔽，而如果刚好消息批次B对应的也是topicA的分区0，那么这个分区已经暂时被屏蔽了，所以消息批次B暂时无法发送
        // 需要等到IO线程将消息批次A发送完毕之后，才会从这个map中取消对这个分区0的屏蔽
        // 因为IO线程发送完请求后 去获取broker的响应时 是会阻塞一段时间 如果在这段时间内获取不到响应，那么就直接略过 进行下一轮的发送
        // 所以可能导致还没收批次A的回复就开始发送批次B
        // 如果消息批次收到的broker传来的响应中包含了异常，而异常又属于可重试异常，并且当前消息批次小于配置的重试次数，那么这个批次就会被放入到对应的
        // 批次队列的队列头，这样就保证了顺序性，而如果重试超过配置的次数又或者说是不可重试异常比如大小超过broker的限制，那么消息批次A最终就不会发送
        // 比如我发送消息A B C 但是B重试了一定次数后依旧失败了，那么最终分区上就只有消息A C 而没有B
        // 但是如果说B在重试的次数内成功发送，那么分区上的消息最终就是A B C
        ...
        // 检测哪些消息批次已经过期了，如果已经过期那么在failBatch方法中会去释放之前申请的批次内存空间，并且触发消息发送失败的回调方法
        // 1.如果消息不是重试消息并且时间已经超过了requestTimeoutMs则过期(因为超出这个时间的如果不能在上面的逻辑中被放入到待发送列表就只能当做过期处理)
        // 2.如果消息是属于过期消息并且时间间隔大于消息失败重试间隔时间retry.backoff.ms
        List<org.apache.kafka.clients.producer.internals.ProducerBatch> expiredBatches = this.accumulator.expiredBatches(this.requestTimeoutMs, now);
        ...
        for (org.apache.kafka.clients.producer.internals.ProducerBatch expiredBatch : expiredBatches) {
            // 调用failBatch使过期的消息批次直接失败(不用再发送到broker)
            // 因为这个failBatch方法在处理broker响应返回异常时也是调用这个方法
            // 这个方法中会去完成之前调用send方法返回的异步任务future，并且将消息批次申请的内存返回给内存缓冲区
            failBatch(expiredBatch, -1, NO_TIMESTAMP, expiredBatch.timeoutException(), false);
            if (transactionManager != null && expiredBatch.inRetry()) {
                // This ensures that no new batches are drained until the current in flight batches are fully resolved.
                // 对过期的消息批次做一个标记，在IO线程的run方法中通过这个标记去判断是否需要重置producerID
                // 比如说消息A和消息B，消息C都属于同一个分区0，消息A的序列号为100，消息B序列号为101，消息C为102
                // 此时消息A发送成功并且在broker上成功写入，消息B发送失败然后重试入队重新等待发送，然后在这个时候发生了超时
                // 那么如果不对消息B做处理，那么消息C就无法发送，因为消息C的序列号为102，而broker上已经写入的消息A的序列号为100
                // 这之间的超过大于1，那么broker会拒绝写入,所以一旦在这边做一个标记，那么IO线程下次轮询时就会去重置生产者的producerID
    ...
    private boolean maybeSendTransactionalRequest(long now) {
        ...
        org.apache.kafka.clients.producer.internals.TransactionManager.TxnRequestHandler nextRequestHandler = transactionManager.nextRequestHandler(accumulator.hasIncomplete());
            ...
            try {
                ...
                if (node != null) {
                    ...
                    if (error == Errors.NONE) {
                        org.apache.kafka.clients.producer.internals.ProducerIdAndEpoch producerIdAndEpoch = new org.apache.kafka.clients.producer.internals.ProducerIdAndEpoch(
     ...
     */
    private void handleProduceResponse(ClientResponse response, Map<TopicPartition, org.apache.kafka.clients.producer.internals.ProducerBatch> batches, long now) {
        ...
        if (response.wasDisconnected()) {
            ...
            for (org.apache.kafka.clients.producer.internals.ProducerBatch batch : batches.values())
        ...
        // API版本导致的错误
            ...
            if (response.hasResponse()) {
                ...
                for (Map.Entry<TopicPartition, ProduceResponse.PartitionResponse> entry : produceResponse.responses().entrySet()) {
                    ...
                    org.apache.kafka.clients.producer.internals.ProducerBatch batch = batches.get(tp);
     ...
     */
    private void completeBatch(org.apache.kafka.clients.producer.internals.ProducerBatch batch, ProduceResponse.PartitionResponse response, long correlationId,
                               long now, long throttleUntilTimeMs) {
        Errors error = response.error;
        // 如果消息批次太大的话 就将其拆分成多个批次 并且重新放入批次队列
        // 而此时申请的批次内存空间  并不是buffer.memory分配的，就是说没这个限制了
        if (error == Errors.MESSAGE_TOO_LARGE && batch.recordCount > 1 &&
            ...
            // canRetry方法中会去判断这个消息批次的重试次数是否小于retries
            // 如果小于 并且异常又属于可重试异常时 调用reenqueueBatch重新进入批次队列
    ...
    private void reenqueueBatch(org.apache.kafka.clients.producer.internals.ProducerBatch batch, long currentTimeMs) {
        this.accumulator.reenqueue(batch, currentTimeMs);
        this.sensors.recordRetries(batch.topicPartition.topic(), batch.recordCount);
    }

    private void completeBatch(org.apache.kafka.clients.producer.internals.ProducerBatch batch, ProduceResponse.PartitionResponse response) {
        if (transactionManager != null) {
    ...
    private void failBatch(org.apache.kafka.clients.producer.internals.ProducerBatch batch, ProduceResponse.PartitionResponse response, RuntimeException exception, boolean adjustSequenceNumbers) {
        failBatch(batch, response.baseOffset, response.logAppendTime, exception, adjustSequenceNumbers);
    }

    private void failBatch(org.apache.kafka.clients.producer.internals.ProducerBatch batch, long baseOffset, long logAppendTime, RuntimeException exception, boolean adjustSequenceNumbers) {
        if (transactionManager != null) {
     ...
     */
    private boolean canRetry(org.apache.kafka.clients.producer.internals.ProducerBatch batch, ProduceResponse.PartitionResponse response) {
     ...
     */
    private void sendProduceRequests(Map<Integer, List<org.apache.kafka.clients.producer.internals.ProducerBatch>> collated, long now) {
        for (Map.Entry<Integer, List<org.apache.kafka.clients.producer.internals.ProducerBatch>> entry : collated.entrySet())
     ...
     */
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<org.apache.kafka.clients.producer.internals.ProducerBatch> batches) {
        if (batches.isEmpty())
            return;

        Map<TopicPartition, MemoryRecords> produceRecordsByPartition = new HashMap<>(batches.size());
        final Map<TopicPartition, org.apache.kafka.clients.producer.internals.ProducerBatch> recordsByPartition = new HashMap<>(batches.size());

        // find the minimum magic version used when creating the record sets
        byte minUsedMagic = apiVersions.maxUsableProduceMagic();
        for (org.apache.kafka.clients.producer.internals.ProducerBatch batch : batches) {
        ...
        // ClientRequest是一个要发送到broker上的请求对象
        // 有几个重要的属性，例如发送目的地，客户端ID，callback等
        ClientRequest clientRequest = client.newClientRequest(nodeId, requestBuilder, now, acks != 0,
                requestTimeoutMs, callback);
        // 此处并不会真正去发送请求，而是将要发送的请求放入待发送的请求中
        client.send(clientRequest, now);
    ...
    public static Sensor throttleTimeSensor(org.apache.kafka.clients.producer.internals.SenderMetricsRegistry metrics) {
    ...
    private class SenderMetrics {
        ...
        private final org.apache.kafka.clients.producer.internals.SenderMetricsRegistry metrics;

        public SenderMetrics(org.apache.kafka.clients.producer.internals.SenderMetricsRegistry metrics) {
        ...
        public void updateProduceRequestMetrics(Map<Integer, List<org.apache.kafka.clients.producer.internals.ProducerBatch>> batches) {
            long now = time.milliseconds();
            for (List<org.apache.kafka.clients.producer.internals.ProducerBatch> nodeBatch : batches.values()) {
                int records = 0;
                for (org.apache.kafka.clients.producer.internals.ProducerBatch batch : nodeBatch) {
