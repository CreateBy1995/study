...
public class KafkaProducer<K, V> implements org.apache.kafka.clients.producer.Producer<K, V> {
    ...
    private final org.apache.kafka.clients.producer.Partitioner partitioner;
    ...
    private final org.apache.kafka.clients.producer.ProducerConfig producerConfig;
    ...
    public KafkaProducer(final Map<String, Object> configs) {
        this(new org.apache.kafka.clients.producer.ProducerConfig(configs), null, null, null, null);
    }
    ...
    public KafkaProducer(Map<String, Object> configs, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
        this(new org.apache.kafka.clients.producer.ProducerConfig(org.apache.kafka.clients.producer.ProducerConfig.addSerializerToConfig(configs, keySerializer, valueSerializer)),
            keySerializer,
            valueSerializer,
            null,
            null);
    ...
    public KafkaProducer(Properties properties) {
        this(new org.apache.kafka.clients.producer.ProducerConfig(properties), null, null, null, null);
    }
    ...
    public KafkaProducer(Properties properties, Serializer<K> keySerializer, Serializer<V> valueSerializer) {
        this(new org.apache.kafka.clients.producer.ProducerConfig(org.apache.kafka.clients.producer.ProducerConfig.addSerializerToConfig(properties, keySerializer, valueSerializer)),
                keySerializer, valueSerializer, null, null);
    }

    @SuppressWarnings("unchecked")
    // visible for testing
    KafkaProducer(org.apache.kafka.clients.producer.ProducerConfig config,
        ...
        try {
            ...
            String clientId = config.getString(org.apache.kafka.clients.producer.ProducerConfig.CLIENT_ID_CONFIG);
            if (clientId.length() <= 0)
                clientId = "producer-" + PRODUCER_CLIENT_ID_SEQUENCE.getAndIncrement();
            this.clientId = clientId;

            String transactionalId = userProvidedConfigs.containsKey(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG) ?
                    (String) userProvidedConfigs.get(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG) : null;
            ...
            MetricConfig metricConfig = new MetricConfig().samples(config.getInt(org.apache.kafka.clients.producer.ProducerConfig.METRICS_NUM_SAMPLES_CONFIG))
                    .timeWindow(config.getLong(org.apache.kafka.clients.producer.ProducerConfig.METRICS_SAMPLE_WINDOW_MS_CONFIG), TimeUnit.MILLISECONDS)
                    .recordLevel(Sensor.RecordingLevel.forName(config.getString(org.apache.kafka.clients.producer.ProducerConfig.METRICS_RECORDING_LEVEL_CONFIG)))
                    .tags(metricTags);
            List<MetricsReporter> reporters = config.getConfiguredInstances(org.apache.kafka.clients.producer.ProducerConfig.METRIC_REPORTER_CLASSES_CONFIG,
                    MetricsReporter.class);
            reporters.add(new JmxReporter(JMX_PREFIX));
            this.metrics = new Metrics(metricConfig, reporters, time);
            ProducerMetrics metricsRegistry = new ProducerMetrics(this.metrics);
            this.partitioner = config.getConfiguredInstance(org.apache.kafka.clients.producer.ProducerConfig.PARTITIONER_CLASS_CONFIG, org.apache.kafka.clients.producer.Partitioner.class);
            long retryBackoffMs = config.getLong(org.apache.kafka.clients.producer.ProducerConfig.RETRY_BACKOFF_MS_CONFIG);
            if (keySerializer == null) {
                this.keySerializer = ensureExtended(config.getConfiguredInstance(org.apache.kafka.clients.producer.ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG,
                                                                                         Serializer.class));
                this.keySerializer.configure(config.originals(), true);
            } else {
                config.ignore(org.apache.kafka.clients.producer.ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG);
                this.keySerializer = ensureExtended(keySerializer);
            }
            if (valueSerializer == null) {
                this.valueSerializer = ensureExtended(config.getConfiguredInstance(org.apache.kafka.clients.producer.ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG,
                                                                                           Serializer.class));
                this.valueSerializer.configure(config.originals(), false);
            } else {
                config.ignore(org.apache.kafka.clients.producer.ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG);
                this.valueSerializer = ensureExtended(valueSerializer);
            }

            // load interceptors and make sure they get clientId
            userProvidedConfigs.put(org.apache.kafka.clients.producer.ProducerConfig.CLIENT_ID_CONFIG, clientId);
            List<org.apache.kafka.clients.producer.ProducerInterceptor<K, V>> interceptorList = (List) (new org.apache.kafka.clients.producer.ProducerConfig(userProvidedConfigs, false)).getConfiguredInstances(org.apache.kafka.clients.producer.ProducerConfig.INTERCEPTOR_CLASSES_CONFIG,
                    org.apache.kafka.clients.producer.ProducerInterceptor.class);
            this.interceptors = new ProducerInterceptors<>(interceptorList);
            ClusterResourceListeners clusterResourceListeners = configureClusterResourceListeners(keySerializer, valueSerializer, interceptorList, reporters);
            this.maxRequestSize = config.getInt(org.apache.kafka.clients.producer.ProducerConfig.MAX_REQUEST_SIZE_CONFIG);
            this.totalMemorySize = config.getLong(org.apache.kafka.clients.producer.ProducerConfig.BUFFER_MEMORY_CONFIG);
            this.compressionType = CompressionType.forName(config.getString(org.apache.kafka.clients.producer.ProducerConfig.COMPRESSION_TYPE_CONFIG));

            this.maxBlockTimeMs = config.getLong(org.apache.kafka.clients.producer.ProducerConfig.MAX_BLOCK_MS_CONFIG);
            this.requestTimeoutMs = config.getInt(org.apache.kafka.clients.producer.ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG);
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    config.getInt(org.apache.kafka.clients.producer.ProducerConfig.BATCH_SIZE_CONFIG),
                    this.totalMemorySize,
                    this.compressionType,
                    config.getLong(org.apache.kafka.clients.producer.ProducerConfig.LINGER_MS_CONFIG),
                    retryBackoffMs,
                    metrics,
                    time,
                    apiVersions,
                    transactionManager);
            List<InetSocketAddress> addresses = ClientUtils.parseAndValidateAddresses(config.getList(org.apache.kafka.clients.producer.ProducerConfig.BOOTSTRAP_SERVERS_CONFIG));
            if (metadata != null) {
                this.metadata = metadata;
            } else {
                this.metadata = new Metadata(retryBackoffMs, config.getLong(org.apache.kafka.clients.producer.ProducerConfig.METADATA_MAX_AGE_CONFIG),
            ...
            KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                    new Selector(config.getLong(org.apache.kafka.clients.producer.ProducerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG),
                            this.metrics, time, "producer", channelBuilder, logContext),
                    this.metadata,
                    clientId,
                    maxInflightRequests,
                    config.getLong(org.apache.kafka.clients.producer.ProducerConfig.RECONNECT_BACKOFF_MS_CONFIG),
                    config.getLong(org.apache.kafka.clients.producer.ProducerConfig.RECONNECT_BACKOFF_MAX_MS_CONFIG),
                    config.getInt(org.apache.kafka.clients.producer.ProducerConfig.SEND_BUFFER_CONFIG),
                    config.getInt(org.apache.kafka.clients.producer.ProducerConfig.RECEIVE_BUFFER_CONFIG),
            ...
            this.sender = new Sender(logContext,
                    ...
                    config.getInt(org.apache.kafka.clients.producer.ProducerConfig.MAX_REQUEST_SIZE_CONFIG),
                    acks,
                    retries,
                    metricsRegistry.senderMetrics,
                    Time.SYSTEM,
                    this.requestTimeoutMs,
                    config.getLong(org.apache.kafka.clients.producer.ProducerConfig.RETRY_BACKOFF_MS_CONFIG),
    ...
    private static TransactionManager configureTransactionState(org.apache.kafka.clients.producer.ProducerConfig config, LogContext logContext, Logger log) {

        TransactionManager transactionManager = null;

        boolean userConfiguredIdempotence = false;
        if (config.originals().containsKey(org.apache.kafka.clients.producer.ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG))
            userConfiguredIdempotence = true;

        boolean userConfiguredTransactions = false;
        if (config.originals().containsKey(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG))
            userConfiguredTransactions = true;

        boolean idempotenceEnabled = config.getBoolean(org.apache.kafka.clients.producer.ProducerConfig.ENABLE_IDEMPOTENCE_CONFIG);

        if (!idempotenceEnabled && userConfiguredIdempotence && userConfiguredTransactions)
            throw new ConfigException("Cannot set a " + org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG + " without also enabling idempotence.");

        if (userConfiguredTransactions)
            idempotenceEnabled = true;

        if (idempotenceEnabled) {
            String transactionalId = config.getString(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG);
            int transactionTimeoutMs = config.getInt(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTION_TIMEOUT_CONFIG);
            long retryBackoffMs = config.getLong(org.apache.kafka.clients.producer.ProducerConfig.RETRY_BACKOFF_MS_CONFIG);
    ...
    private static int configureRetries(org.apache.kafka.clients.producer.ProducerConfig config, boolean idempotenceEnabled, Logger log) {
        boolean userConfiguredRetries = false;
        if (config.originals().containsKey(org.apache.kafka.clients.producer.ProducerConfig.RETRIES_CONFIG)) {
        ...
        if (idempotenceEnabled && config.getInt(org.apache.kafka.clients.producer.ProducerConfig.RETRIES_CONFIG) == 0) {
            throw new ConfigException("Must set " + org.apache.kafka.clients.producer.ProducerConfig.RETRIES_CONFIG + " to non-zero when using the idempotent producer.");
        }
        return config.getInt(org.apache.kafka.clients.producer.ProducerConfig.RETRIES_CONFIG);
    }

    private static int configureInflightRequests(org.apache.kafka.clients.producer.ProducerConfig config, boolean idempotenceEnabled) {
        if (idempotenceEnabled && 5 < config.getInt(org.apache.kafka.clients.producer.ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION)) {
            throw new ConfigException("Must set " + org.apache.kafka.clients.producer.ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION + " to at most 5" +
                    " to use the idempotent producer.");
        }
        return config.getInt(org.apache.kafka.clients.producer.ProducerConfig.MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION);
    }

    private static short configureAcks(org.apache.kafka.clients.producer.ProducerConfig config, boolean idempotenceEnabled, Logger log) {
        boolean userConfiguredAcks = false;
        short acks = (short) parseAcks(config.getString(org.apache.kafka.clients.producer.ProducerConfig.ACKS_CONFIG));
        if (config.originals().containsKey(org.apache.kafka.clients.producer.ProducerConfig.ACKS_CONFIG)) {
            userConfiguredAcks = true;
        }

        if (idempotenceEnabled && !userConfiguredAcks) {
            log.info("Overriding the default {} to all since idempotence is enabled.", org.apache.kafka.clients.producer.ProducerConfig.ACKS_CONFIG);
            return -1;
        }

        if (idempotenceEnabled && acks != -1) {
            throw new ConfigException("Must set " + org.apache.kafka.clients.producer.ProducerConfig.ACKS_CONFIG + " to all in order to use the idempotent " +
    ...
    public Future<org.apache.kafka.clients.producer.RecordMetadata> send(org.apache.kafka.clients.producer.ProducerRecord<K, V> record, org.apache.kafka.clients.producer.Callback callback) {
        // 调用自定义拦截器的onSend方法 在onSend方法中我们可以对消息进行一些修改
        org.apache.kafka.clients.producer.ProducerRecord<K, V> interceptedRecord = this.interceptors.onSend(record);
     ...
     */
    private Future<org.apache.kafka.clients.producer.RecordMetadata> doSend(org.apache.kafka.clients.producer.ProducerRecord<K, V> record, Callback callback) {
        // TopicPartition表示了消息对应的话题和分区 这个对象有两个属性，String topic表示话题，int partition表示第几个分区
        TopicPartition tp = null;
        try {
            // 1、检测IO线程是否还存活
            throwIfProducerClosed();
            // 2、在waitOnMetadata这个方法中 首先会查看是有对应topic的cluster信息记录，cluster表示topic在集群中的元数据信息，记录了集群的节点信息 以及topic对应的分区信息等
            // 如果没有记录的话会去kafka集群上获取，waitTime表示去kafka集群上获取topic对应的信息花费的时间
            // 如果在获取过程中waitTime时间超过了此处的maxBlockTimeMs(max.block.ms,默认60s，可配置) 那么就会抛出异常
            // 而如果topic已经有记录过了 那么直接返回 并且waitTime为0 表示没有花费时间
            ...
            // 剩余可执行时间，其实也就是说发送一条数据最长的可用时间是maxBlockTimeMs(max.block.ms)，如果发送数据的流程没有在这个时间内完成，就会抛出异常
            // 此处的可用时间是指从我们创建一个消息到调用send方法加入到批次队列的时间，不包括IO线程发送这个批次到broker的时间
            // kafka生产者发送的时间包括2个阶段，一是用户创建消息调用send加入到批次队列，二是IO线程从批次队列中取出这个批次发送到kafka
            // 也就是说此处的时间限制是对第一阶段的限制
            ...
            try {
                ...
                throw new SerializationException("Can't convert key of class " + record.key().getClass().getName() +
                        " to class " + producerConfig.getClass(org.apache.kafka.clients.producer.ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG).getName() +
            ...
            try {
                ...
                throw new SerializationException("Can't convert value of class " + record.value().getClass().getName() +
                        " to class " + producerConfig.getClass(org.apache.kafka.clients.producer.ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG).getName() +
                        " specified in value.serializer", cce);
            }
            // 4、通过partition方法来获取消息要发送的分区，如果消息没有指定分区
            // 那么kafka默认会将消息的key进行hash后与总分区数进行取模
            // 如果消息的key为空，那么kafka会随机一个数，然后每次将这个数递增后与总分区数进行取模
            int partition = partition(record, serializedKey, serializedValue, cluster);
            tp = new TopicPartition(record.topic(), partition);

            setReadOnly(record.headers());
            Header[] headers = record.headers().toArray();
            // 5、此处会估算一下消息序列化后的大小，之所以是估算 因为此处还没有去考虑压缩算法的开销
            ...
            // 如果返回的结果是累加器已经满了，或者新的累加器被创建，那么就唤醒IO线程 使其发送累加器中的消息
        ...
        } catch (org.apache.kafka.clients.producer.BufferExhaustedException e) {
            this.errors.record();
     ...
     */
    private int partition(org.apache.kafka.clients.producer.ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
    ...
    private void throwIfNoTransactionManager() {
        if (transactionManager == null)
            throw new IllegalStateException("Cannot use transactional methods without enabling transactions " +
                    "by setting the " + org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG + " configuration property");
    }
    ...
    private static class FutureFailure implements Future<org.apache.kafka.clients.producer.RecordMetadata> {
        ...
        @Override
        public org.apache.kafka.clients.producer.RecordMetadata get() throws ExecutionException {
            throw this.exception;
        }

        @Override
        public org.apache.kafka.clients.producer.RecordMetadata get(long timeout, TimeUnit unit) throws ExecutionException {
            throw this.exception;
        }
    ...
    private static class InterceptorCallback<K, V> implements Callback {
        ...
        public void onCompletion(org.apache.kafka.clients.producer.RecordMetadata metadata, Exception exception) {
            metadata = metadata != null ? metadata : new org.apache.kafka.clients.producer.RecordMetadata(tp, -1, -1, RecordBatch.NO_TIMESTAMP, Long.valueOf(-1L), -1, -1);
